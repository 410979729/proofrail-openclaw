...
exports.isLikelyMutatingExec = isLikelyMutatingExec;
exports.isLikelyValidationExec = isLikelyValidationExec;
exports.getExecCommand = getExecCommand;
const constants_1 = require("./constants");
function hasShortFlag(command, flag) {
    return new RegExp(`(^|\\s)-[a-z]*${flag}[a-z]*(?=\\s|$)`, "i").test(command);
}
function hasLongFlag(command, flag) {
...
function parseQuotedPayload(command, startIndex, quote) {
...
function splitShellOptionToken(token) {
    if (!token.startsWith("-"))
        return { name: token };
...
function shortOptionTokenConsumesNextValue(token) {
    if (!/^[-+][A-Za-z]+$/.test(token))
        return false;
    return token.includes("o") || token.includes("O");
}
function shellOptionConsumesValue(token) {
    return token === "-o"
        || token === "+o"
        || token === "-O"
        || token === "+O"
...
function getNestedShellPayloads(command) {
    const payloads = [];
    ...
    while ((match = nestedShellPattern.exec(command)) !== null) {
        ...
        while (index < command.length) {
            ...
            if (!sawCommandFlag) {
                ...
                if (shellOption.name.startsWith("-"))
                    continue;
                break;
            }
...
function isLikelyScriptedFixExec(command) {
    if (!/\b(npm|pnpm|yarn|bun)\s+(?:run\s+)?[^\s]+\b/i.test(command))
        return false;
    return /(^|\s)--(?:fix|write)(?=\s|$)/i.test(command);
}
function getDangerousRmLabel(command) {
    ...
    if (/(^|\s)(["'])?\/(?:\*+)?\2(?=\s|$)/.test(command))
        return "rm -rf /";
    if (/(^|\s)(["'])?\/\.\*(?:\/(?:\*+)?)?\2(?=\s|$)/.test(command))
        return "rm -rf root hidden files";
    if (/(^|\s)\.\[!\.\]\*\s+\.\.\?\*\s+\*(?=\s|$)/.test(command))
        return "rm -rf all current directory contents";
    if (/(^|\s)(["'])?\*(?:\/(?:\*+)?)?\2(?=\s|$)/.test(command))
        return "rm -rf current directory contents";
    if (/(^|\s)(["'])?\.\/\*(?:\/(?:\*+)?)?\2(?=\s|$)/.test(command))
        return "rm -rf current directory contents";
    if (/(^|\s)(["'])?\.\*(?:\/(?:\*+)?)?\2(?=\s|$)/.test(command))
        return "rm -rf hidden files";
    if (/(^|\s)(["'])?\.\/\.\*(?:\/(?:\*+)?)?\2(?=\s|$)/.test(command))
        return "rm -rf hidden files";
    if (/(^|\s)(["'])?\.(?:\/(?:\*+)?)?\2(?=\s|$)/.test(command))
        return "rm -rf current directory";
    if (/(^|\s)(["'])?\$PWD\2(?:\/(?:\*)?)?(?=\s|$)/.test(command))
        return "rm -rf $PWD";
    if (/(^|\s)(["'])?\$\{PWD\}\2(?:\/(?:\*)?)?(?=\s|$)/.test(command))
        return "rm -rf ${PWD}";
    if (/\$\(\s*pwd\s*\)(?:\/(?:\*)?)?/i.test(command))
        return "rm -rf $(pwd)";
    if (/(^|\s)(["'])?\$HOME\2(?:\/(?:\*)?)?(?=\s|$)/.test(command))
        return "rm -rf $HOME";
    if (/(^|\s)(["'])?\$\{HOME\}\2(?:\/(?:\*)?)?(?=\s|$)/.test(command))
        return "rm -rf ${HOME}";
    if (/(^|\s)(["'])?~(?:\/(?:\*+)?)?\2(?=\s|$)/.test(command))
        return "rm -rf home directory";
    if (/(^|\s)(["'])?\.\.(?:\/[^\s"']*)?\2(?=\s|$)/.test(command))
        return "rm -rf parent directory";
    if (/(^|\s)(["'])?\.\/\.\.(?:\/[^\s"']*)?\2(?=\s|$)/.test(command))
        return "rm -rf parent directory";
    return undefined;
}
function getDangerousGitCleanLabel(command) {
    ...
    if (!hasForce || !hasDir || dryRun)
        return undefined;
    if (hasIgnored)
        return "git clean -fdx";
    return "git clean -fd";
}
function isFindPreludeOption(token) {
    return /^(?:-H|-L|-P)$/.test(token)
        || /^-O\d*$/.test(token)
        || token === "-D";
}
function getFindStartPoints(command) {
...
function getDangerousFindStartPointLabel(target, operation) {
    if (/^\/$/.test(target))
        return `find / ${operation}`;
    if (/^\.(?:\/)?$/.test(target) || /^\.\/\.(?:\/)?$/.test(target))
        return `find . ${operation}`;
    if (/^\.\.(?:\/)?$/.test(target) || /^\.\/\.\.(?:\/)?$/.test(target))
        return `find parent directory ${operation}`;
    if (/^\$PWD\/?$/.test(target))
        return `find $PWD ${operation}`;
    if (/^\$\{PWD\}\/?$/.test(target))
        return "find ${PWD} " + operation;
    if (/^\$HOME\/?$/.test(target))
        return `find $HOME ${operation}`;
    if (/^\$\{HOME\}\/?$/.test(target))
        return "find ${HOME} " + operation;
    if (/^~\/?$/.test(target))
        return `find home directory ${operation}`;
    return undefined;
}
function getDangerousFindDeleteLabel(command) {
    if (!/\bfind\b/i.test(command))
        return undefined;
    if (!/(^|\s)-delete(?=\s|$)/i.test(command))
        return undefined;
...
function getDangerousFindExecRmLabel(command) {
    if (!/\bfind\b/i.test(command))
        return undefined;
    if (!/(^|\s)-exec(?:dir)?\s+(?:\/bin\/)?rm\b/i.test(command))
        return undefined;
    for (const target of getFindStartPoints(command)) {
...
function getDangerousXargsRmLabel(command) {
...
function getDangerousScriptOneLinerLabel(command) {
...
function getDangerousScriptHereDocLabel(command) {
...
function getDangerousRemoteScriptLabel(command) {
...
function getSshRemotePayloads(command) {
    const payloads = [];
...
function hasToolDryRunFlag(command, tool) {
    if (tool === "kubectl")
        return /(^|\s)--dry-run(?:=\S+)?(?=\s|$)/i.test(command);
    return /(^|\s)--dry-run(?=\s|$)/i.test(command);
}
function getDangerousInfrastructureLabel(command) {
    if (/\bterraform\s+destroy\b/i.test(command))
        return "terraform destroy";
    if (/\bkubectl\s+delete\s+(?:namespace|ns|all|clusterrole|clusterrolebinding)\b/i.test(command) && !hasToolDryRunFlag(command, "kubectl"))
        return "kubectl destructive delete";
    if (/\bhelm\s+(?:uninstall|delete)\b/i.test(command) && !hasToolDryRunFlag(command, "helm"))
        return "helm uninstall/delete";
    if (/\bdocker\s+(?:system|volume|image|container|builder)\s+prune\b/i.test(command))
        return "docker prune";
    return undefined;
}
...
function hasShellWriteRedirection(command) {
    ...
    while ((match = redirectionPattern.exec(command)) !== null) {
        ...
        if (/^&\d+$/.test(target))
            continue;
        if (isBenignWriteSink(target))
            continue;
        return true;
    }
    return false;
}
function hasTeeWrite(command) {
...
function isReadOnlyFsProbe(command) {
...
function isLikelyFileWritingExec(command) {
...
function isLikelyScriptOneLinerMutation(command) {
...
function isLikelyFindDeleteExec(command) {
    return /\bfind\b/i.test(command) && /(^|\s)-delete(?=\s|$)/i.test(command);
}
function isLikelyFindExecRm(command) {
    return /\bfind\b/i.test(command) && /(^|\s)-exec(?:dir)?\s+(?:\/bin\/)?rm\b/i.test(command);
}
function isLikelyXargsRm(command) {
    return /\bxargs\b[\s\S]*\brm\b/i.test(command);
}
function isKnownDryRunInfrastructureCommand(command) {
    return (/\bkubectl\b/i.test(command) && hasToolDryRunFlag(command, "kubectl"))
        || (/\bhelm\b/i.test(command) && hasToolDryRunFlag(command, "helm"));
}
function isLikelyInfrastructureMutation(command) {
    if (/\bkubectl\s+(?:apply|delete|replace|patch|scale|cordon|uncordon|drain)\b/i.test(command) && !hasToolDryRunFlag(command, "kubectl"))
        return true;
    if (/\bkubectl\s+rollout\s+restart\b/i.test(command))
        return true;
    if (/\bhelm\s+(?:install|upgrade|uninstall|delete|rollback)\b/i.test(command) && !hasToolDryRunFlag(command, "helm"))
        return true;
...
function isLikelyRemoteScriptExec(command) {
    return Boolean(getDangerousRemoteScriptLabel(command));
}
function isLikelyGitCleanMutation(command) {
    if (!/\bgit\s+clean\b/i.test(command))
        return undefined;
    const hasForce = hasShortFlag(command, "f") || hasLongFlag(command, "force");
    const dryRun = hasShortFlag(command, "n") || hasLongFlag(command, "dry-run");
...
function isDangerousCommandAtDepth(command, depth) {
    if (depth > 4)
        return { dangerous: false };
    for (const nestedPayload of getNestedShellPayloads(command)) {
        if (nestedPayload === command)
            continue;
        const nestedResult = isDangerousCommandAtDepth(nestedPayload, depth + 1);
        if (nestedResult.dangerous)
            return nestedResult;
    }
    const fullDangerousScriptLabel = getDangerousScriptOneLinerLabel(command);
    if (fullDangerousScriptLabel)
        return { dangerous: true, label: fullDangerousScriptLabel };
    const fullDangerousHereDocLabel = getDangerousScriptHereDocLabel(command);
    if (fullDangerousHereDocLabel)
        return { dangerous: true, label: fullDangerousHereDocLabel };
    const fullRemoteScriptLabel = getDangerousRemoteScriptLabel(command);
    if (fullRemoteScriptLabel)
        return { dangerous: true, label: fullRemoteScriptLabel };
    const fullInfrastructureLabel = getDangerousInfrastructureLabel(command);
    if (fullInfrastructureLabel)
        return { dangerous: true, label: fullInfrastructureLabel };
    for (const remotePayload of getSshRemotePayloads(command)) {
        ...
        if (remoteResult.dangerous)
            return { dangerous: true, label: `ssh remote: ${remoteResult.label || "dangerous command"}` };
    }
    for (const segment of splitShellSegments(command)) {
        for (const nestedPayload of getNestedShellPayloads(segment)) {
            if (nestedPayload === segment)
                continue;
            const nestedResult = isDangerousCommandAtDepth(nestedPayload, depth + 1);
            if (nestedResult.dangerous)
                return nestedResult;
        ...
        if (dangerousRemoteScriptLabel)
            return { dangerous: true, label: dangerousRemoteScriptLabel };
        const dangerousInfrastructureLabel = getDangerousInfrastructureLabel(segment);
        if (dangerousInfrastructureLabel)
            return { dangerous: true, label: dangerousInfrastructureLabel };
        ...
        if (dangerousRmLabel)
            return { dangerous: true, label: dangerousRmLabel };
        const dangerousFindDeleteLabel = getDangerousFindDeleteLabel(segment);
        if (dangerousFindDeleteLabel)
            return { dangerous: true, label: dangerousFindDeleteLabel };
        const dangerousFindExecRmLabel = getDangerousFindExecRmLabel(segment);
        if (dangerousFindExecRmLabel)
            return { dangerous: true, label: dangerousFindExecRmLabel };
        const dangerousXargsRmLabel = getDangerousXargsRmLabel(segment);
        if (dangerousXargsRmLabel)
            return { dangerous: true, label: dangerousXargsRmLabel };
        const dangerousScriptLabel = getDangerousScriptOneLinerLabel(segment);
        if (dangerousScriptLabel)
            return { dangerous: true, label: dangerousScriptLabel };
        const dangerousHereDocLabel = getDangerousScriptHereDocLabel(segment);
        if (dangerousHereDocLabel)
            return { dangerous: true, label: dangerousHereDocLabel };
        const dangerousGitCleanLabel = getDangerousGitCleanLabel(segment);
        if (dangerousGitCleanLabel)
            return { dangerous: true, label: dangerousGitCleanLabel };
        for (const { re, label } of constants_1.DANGEROUS_PATTERNS) {
            if (re.test(segment)) {
...
function isLikelyMutatingExecAtDepth(command, depth) {
    if (depth > 4)
        return false;
    for (const nestedPayload of getNestedShellPayloads(command)) {
        if (nestedPayload !== command && isLikelyMutatingExecAtDepth(nestedPayload, depth + 1))
            return true;
    }
    for (const remotePayload of getSshRemotePayloads(command)) {
        if (isLikelyMutatingExecAtDepth(remotePayload, depth + 1))
            return true;
    }
    if (isLikelyScriptedFixExec(command))
        return true;
    if (isLikelyRemoteScriptExec(command))
        return true;
    if (isLikelyInfrastructureMutation(command))
        return true;
    if (isLikelyFileWritingExec(command))
        return true;
    if (isLikelyFindDeleteExec(command))
        return true;
    if (isLikelyFindExecRm(command))
        return true;
    if (isLikelyXargsRm(command))
        return true;
    if (isLikelyScriptOneLinerMutation(command))
        return true;
    const fullGitCleanMutation = isLikelyGitCleanMutation(command);
    if (typeof fullGitCleanMutation === "boolean")
        return fullGitCleanMutation;
    for (const segment of splitShellSegments(command)) {
        for (const nestedPayload of getNestedShellPayloads(segment)) {
            if (nestedPayload !== segment && isLikelyMutatingExecAtDepth(nestedPayload, depth + 1))
                return true;
        }
        for (const remotePayload of getSshRemotePayloads(segment)) {
            if (isLikelyMutatingExecAtDepth(remotePayload, depth + 1))
        ...
        if (isLikelyScriptedFixExec(segment))
            return true;
        if (isLikelyRemoteScriptExec(segment))
            return true;
        if (isLikelyInfrastructureMutation(segment))
            return true;
        if (isLikelyFileWritingExec(segment))
            return true;
        if (isLikelyFindDeleteExec(segment))
            return true;
        if (isLikelyFindExecRm(segment))
            return true;
        if (isLikelyXargsRm(segment))
            return true;
        if (isLikelyScriptOneLinerMutation(segment))
            return true;
        const gitCleanMutation = isLikelyGitCleanMutation(segment);
        if (typeof gitCleanMutation === "boolean")
            return gitCleanMutation;
        if (isReadOnlyFsProbe(segment))
            continue;
        if (!isKnownDryRunInfrastructureCommand(segment) && constants_1.MUTATING_EXEC_PATTERNS.some((pattern) => pattern.test(segment)))
            return true;
    }
    return false;
}
...
function isLikelyMutatingExec(command) {
    return isLikelyMutatingExecAtDepth(command, 0);
}
function isLikelyValidationExec(command) {
    if (isLikelyMutatingExec(command))
        return false;
    if (constants_1.VALIDATION_EXEC_PATTERNS.some((pattern) => pattern.test(command)))
        return true;
    return /\b(curl|wget)\b/i.test(command) && constants_1.VALIDATION_ENDPOINT_HINTS.test(command);
}
function getExecCommand(input) {
